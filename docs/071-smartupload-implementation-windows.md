...
Если с течением времени возникла необходимость поменять порт, через который должен будет работать ПК, то после его замены 
в административной части самого ПК, актуальный порт также необходимо прописать и в этом скрипте вручную.

---

***ВАЖНО!!!*** Если скрипт размещается на физически той же машине, на которой развернут ПК ТЭ, то IP-адрес следует писать 127.0.0.1:ПОРТ.
Не следует извращаться с вписываением IP-адреса из пула адресов локальной сети, равно как не следует писать ИМЯ-МАШИНЫ:ПОРТ, так как 
могут быть разного рода ограничения и неожиданности - локальные IP-адреса меняются очень часто сервером.
