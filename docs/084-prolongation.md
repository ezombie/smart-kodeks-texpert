...
**ВНИМАНИЕ** Система выбирает данные за длительный период времени и данные в ячейчах могут появляться с некоторой
задержкой. Обычно это не более 30 секунд. Если появляется треугольный значёк с восклицательным знаком - обновите
страницу. В случае невозможности получить данные - обратитесь в канал поддержки.
...
Под таблице есть актуальная инструкция по использованию.
...
Формула рассчёта значений "% ХХХХХХ к предыдущему ":  **(Предыдущее - Текущее) / (Текущее / 100 )**. Если
...
При составлении отчёта кроме сухих цифр количества отвергнутых подключений рекомендуется обратиться к дашборду
"Для менеджеров" к таблице со списком пользователей которые не могли подключиться к системе (содержит время когда это произошло)

### Статистика работы KAssist

Для обогащения данными по работы подсистемы **kassist** рекомендуется обратиться к дашборду "kAssist. Аналитика" 
