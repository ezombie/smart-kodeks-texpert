...
- изменять временной отрезок, за который необходимо отразить данные [Как это сделать](042-grafana-visualization#управление-временем-в-дашбордах);
- изменять отображаемые установки, данные по которым требуется проанализировать [Как это сделать](042-grafana-visualization#использование-grafana-для-визуализации-данных); 
...
При обнаружении аномально резкого снижения количества пользователей (особенно вкупе с низкими показателями из графика [Используемые лицензии](062-kodeks-analytics#используемые-лицензии)) рекомендуется незамедлительно выяснить причины такого поведения и, опционально, своевременно отработать вторжение конкурента, если таковой будет иметь место быть.
...
Рекомендуется при оценке использовать данные с графиков [Запросы к БД](062-kodeks-analytics#запросы-к-бд), [Используемые лицензии](062-kodeks-analytics#используемые-лицензии), [Doc or service](062-kodeks-analytics#doc-or-service), а также с дашбордов Подключенные пользователи и "Поисковые запросы".
...
- график [Не пользуются системой более 60 дней](062-kodeks-analytics#не-пользуются-системой-в-течение-60-дней) с целью выявить новых-старых пользователей, решить их проблемы неиспользования ТЭ и возобновить интерес к системе;
- график [Используемые лицензии](062-kodeks-analytics#используемые-лицензии) с целью учета данных при рассмотрении вопроса о пересмотре состава обслуживания;
...
- нырять [дальше](063-billing-online-statistics) в информационные глубины СМАРТа; 
- вернуться [назад](062-kodeks-analytics#kodeks-аналитика);
- вернуться совсем к [началу](060-dashboards#дашборды), чтобы выборочно перейти к другому подразделу;
