...
немнооооого совсем погрузиться в тему мониторинга в принципе и зачем он нужен как таковой.

Тема мониторинга сама по себе родилась практически сразу как были созданы компьютерные сети, т.е. к середине 20 века. 
Причина тому - лень. Как известно, лень - двигатель прогресса. Речь тут, в том числе, о заре Интернета, так как компьютеры, 
объединяемые в сети, требовали постоянного надзора за собой, как с аппаратной стороны, так и с программной. И если один 
отдельно стоящий компьютер, еще можно было адекватно контролировать вручную (не без нюансов, конечно, но можно), то распределенные 
сети компьютеров уже требовали гораздо более оперативного контроля за собой, чем человек мог обеспечить "в ручную".
...
На текущий момент систем мониторинга существует достаточно, чтобы закрывать человеческую потребность в контроле за своим
детищем (т.е. за распределенными сетями компьютеров и ПО на них) практически на любой вкус и цвет и размер и назначение.
Возможно даже на твоем предприятии, коллега, также существует своя система мониторинга.

На этом моменте есть важное отступление: коллега, у нас с тобой есть наше любимое АСРП. И поэтому нужно четко разделить вот какой момент:
- АСРП это система построения отчетов на основании загруженных в нее отчетов sysinfo (и другой функционал, но в рамках
данной статьи нам важен именно этот ее функционал)
- СМАРТ-мониторинг это система проактивного контроля в live-режиме за конкретными объектами (адресами, установками)

Вывод - АСРП и СМАРТ-мониторинг это совершенно разные системы ввиду из разного назначения и, как следствие, разных сценариев использования.

Это необходимо понимать и держать в голове.

## Зачем нужен мониторинг?

Как было сказано выше - распределенные сети компьютеров требуют неусыпного контроля. Сеть компьютеров в рамках офиса предприятия
также называют инфраструктурой. И когда нет мониторинга за такой инфраструктурой, то проблемы с ней становятся заметны
только тогда когда проблемы уже случились и оказали свое негативное влияние на всю инфраструктуру: сервисы внутри
инфраструктуры тормозя или вообще недоступны, пользователя не могут пользоваться такими сервисами или вообще не могут быть
во всей инфраструктуре. Команда ответственных специалистов реагирует на возникающие проблемы вслепую, лечит "симптомы"
проблемы, не имея возможности разобраться в причинах. В таких условиях работа превращается в постоянное «тушение пожаров».

Для бизнеса, тем более дистрибьюторского бизнеса, это не просто "некоторое неудобство". Это буквально потерянные деньги
от клиентов, испорченный пользовательский опыт, подмоченная репутация, падение доверия и т.д. Чем сложнее система, тем 
дороже обходится любая задержка в её работе.

Мониторинг меняет правила игры. 
Рассмотрим несколько примеров:
- **Предотвращает проблемы до жалоб пользователей.** 
Допустим, у приложения постепенно растёт время отклика. Без мониторинга инженеры узнают о проблеме только из жалоб в поддержку 
(в лучшем случае). С мониторингом система подаст сигнал ещё на этапе роста задержек, и проблему можно исправить до того, 
как пользователи её заметят. Аналогично — утечка памяти или заполнение диска: уведомление о возникающей проблеме приходит 
заранее, и команда реагирует до полного сбоя.
- **Экономит ресурсы команды**. 
Инженеры без мониторинга работают реактивно: чинят то, что уже сломалось и тогда, когда действовать нужно быстро и от 
твоих действий зависит последующая доступность сервиса. Мониторинг позволяет исправлять небольшие, только возникающие 
проблемы, не давая перерасти им в масштабные инциденты.
- **Даёт историческую картину.** 
Метрики не просто фиксируют «что сейчас», они позволяют видеть, как показатели менялись в прошлом. Например, можно заметить 
постепенное падение производительности базы данных или рост числа ошибок за последние месяцы. Это помогает выявлять 
неочевидные тенденции или аномалии и строить прогнозы на ближайшую перспективу с целью дальнейшего построения планов развития.
- **Помогает масштабировать инфраструктуру.**
Например, если каждый декабрь трафик сервиса резко возрастает перед праздниками, исторические данные позволят 
заранее рассчитать, сколько ресурсов понадобится, и подготовить инфраструктуру, избегая перегрузок и аварийных падений.
- **Снижает бизнес-риски.**
Чем быстрее команда реагирует на проблемы, тем меньше времени система/сервис остаётся недоступной. Это напрямую влияет на доход 
и репутацию компании: минута простоя может стоить огромных сумм в разной валюте. Мониторинг минимизирует эти потери или
вообще сводит к нулю.

**Отсюда следует простой вывод** - построенная **система мониторинга** это не просто технический инструмент для инженеров. 
Он **помогает поддерживать стабильную работу сервисов, обеспечивает предсказуемость и прозрачность, а также позволяет 
работать без постоянного страха перед очередным сбоем**.

---
## Зачем нужен СМАРТ-Мониторинг?
---
...
- Система автоматизированного сбора статистических данных (здесь и далее — отчетов sysinfo и smartstatus) с подконтрольных СМАРТу объектов; 
- Система оповещений; 
