...
## Автоматизация исполнения скрипта smartupload.sh

После того как станет точно ясно, что тестовый запуск скрипт прошел успешно, необходимо автоматизировать исполнение скрипта.
Для этого используется стандартный инструмент Cron.

Cron — популярный инструмент в Unix-системах, который помогает запланировать исполнение различных заданий прямо в ОС.
Коротко cron можно описать как планировщик задач. Он способен в обозначенное время с определенной периодичностью выполнять 
скрипты на сервере.
Файл crontab представляет собой таблицу, оформленную в специальном формате и содержащую расписание старта скриптов. 
Каждый пользователь на сервере системы получает доступ к индивидуальному crontab-файлу, в котором создает нужные ему задачи. 
Cron является встроенной утилитой и доступен в каждом дистрибутиве Linux.

Для того чтобы создать задачу на регулярное исполнение скрипта smartupload.sh, нужно выполнить следующие действия:
1. открыть окно консоли
2. набрать crontab -e (откроется таблица заданий для текущего пользователя или будет создана и открыта чистая таблица, если ранее ее не существовало)
3. нужно чтобы скрипт smartupload.sh исполнялся ежечасно ежедневно постоянно, соответственно, в crontab необходимо внести
следующую запись:

@hourly /opt/kodeks/intranet/smartupload/smartupload.sh

Записать изменения в файл, нажав Ctrl+O

В примере выше путь до скрипта указан примерный, необходимо подставить настоящий.

В случае затруднений с этим этапом, можно делегировать ее системному администратору клиента, сообщив ему сущность того
что надо сделать и/или спросить советов и консультаций у разработчика СМАРТ-Мониторинга.
